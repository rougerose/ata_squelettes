    ...
    _createClass(AtlasBase, [{
      ...
      value: function configMap() {
        ...
        var onready = function onready() {
          _this2._handleClickMarker(); // Si un marker est appelé explicitement à l'ouverture :
          // - ajuster la carte au centre
          // - et ouvrir modalAssociation


          if (_this2.map.options.openId) {
            var id = _this2.map.options.openId;

            _this2.centerOnMarker(id);

            _this2.dispatch({
              type: "addModalContent",
              openId: _this2.map.options.openId,
      ...
      key: "centerOnMarker",
      value: function centerOnMarker(id) {
        var self = this;
        var marker = this.memory.markers[id];
        this.map.markerCluster.zoomToShowLayer(marker, function () {
          ...
          var clusterBounds = marker.__parent.getBounds();

          var zoomLevel = self.map.getBoundsZoom(clusterBounds); // console.log(zoomLevel);
          // Ajouter un padding en version desktop

          if (self.state.windowWidth === "desktop") {
            var searchboxWidth = self.controls.SearchBox.getWidth();
      ...
      value: function searchCollection(keywords) {
        ...
        collection.fail(function (jqxhr, textStatus, error) {
          associations.id_association.push(0); // var err = textStatus + ", " + error;
      ...
      value: function searchFeatures(url, args) {
        ...
        jQuery.getJSON(url, args, function (data) {
          if (data) {
            map.removeAllMarkers(); // centrer la carte sur les marqueurs correspondant à la recherche
            // relancer le gestionnaire de click sur les marqueurs.

            var coords = [];
            coords = data.features.map(function (feature) {
              return feature.geometry.coordinates.slice().reverse();
            });
