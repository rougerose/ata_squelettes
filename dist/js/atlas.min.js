var atlas = (function (exports) {
  ...
  var AtlasBase = /*#__PURE__*/function () {
    function AtlasBase(map, state, conf) {
      ...
      this.map = map;
      this.configMap();
      this.handleResize = this._handleResize.bind(this); // Ajouter gestionnaire sur window.onresize
      ...
      this.container = container; // Activer les modules "controls"
    ...
    _createClass(AtlasBase, [{
      ...
      value: function configMap() {
        ...
        var onready = function onready() {
          _this2._handleClickMarker();

          jQuery("#" + _this2.map._container.id).off("ready", onready);
        };
      ...
      value: function handleAction(state, action) {
        var keywords = new Map(state.keywords);
        var keywordsSelected = new Map(state.keywordsSelected);
  ...
  */
  ...
  var KeywordSelected = /*#__PURE__*/function () {
    ...
    _createClass(KeywordSelected, [{
      ...
      value: function syncState(state) {
        if (state.keywordsSelected.size !== this.state.keywordsSelected.size) {
          ...
          if (state.keywordsSelected.size > 0) {
            ...
            var _iterator = _createForOfIteratorHelper(state.keywordsSelected),
                _step;
      ...
      value: function removeKeyword(el, removeParent) {
        ...
        var tippyBtn = el.querySelector("." + config.keywords.tooltipClassName); // Si un tooltip est présent, supprimer l'instance tippy
  ...
  var Modal = /*#__PURE__*/function () {
    function Modal(state, _ref) {
      ...
      this.memory = {};
      var modal = this.getModal(this.settings.containerId);
      modal.setAttribute("aria-hidden", "true");
      this._handleClickClose = this.handleClickClose.bind(this);
      this._handleClickToggle = this.handleClickToggle.bind(this); // this._handleCancel = this._handleCancel.bind(this);
    }
    ...
    _createClass(Modal, [{
          ...
          return _invoke(function () {
            if (state.modal.action) {
              return _invokeIgnored(function () {
                if (state.modal.action === "open" && _this2.state.modal.position !== "closed") {
                  console.log("close");
                  return _await(_this2.close(), function () {
                    _this2.open(state.modal.openId);
                  });
            ...
            } else if (state.windowWidth === "desktop" && _this2.state.modal.position === "openedFull" && state.searchboxHeight !== _this2.state.searchboxHeight) {
              console.log("update", state);

              _this2.updateLayout(state.searchboxHeight);
            }
          }, function () {
            _this2.state = state;
          });
      ...
      value: function getModal() {
        var id = this.settings.containerId;
      ...
      value: function updateLayout(value) {
        ...
        var layout = modal.firstElementChild.firstElementChild;
        layout.style.paddingTop = value + "px";
      }
    }, {
      key: "animBtn",
      value: function animBtn(button) {
      ...
      value: function close() {
        try {
          ...
          var modal = _this5.getModal();

          return _await(_this5.closeTransition(modal, _this5.memory.settings), function () {
            _this5.unbindListeners(modal);
      ...
      value: function handleOpen() {
        try {
          ...
          var modal = _this7.getModal(); // Gestionnaires d'événements


          _this7.bindListeners(modal); // Définir des variables supplémentaires


          _this7.memory.settings = {};
          var settings = _this7.memory.settings;

          if (_this7.state.windowWidth === "desktop") {
            _this7.updateLayout(_this7.state.searchboxHeight);

            settings.position = "openedFull"; // translation depuis la gauche de la fenêtre

            settings.transitionOn = "translateX(0)";
            settings.transitionOff = "translateX(-100%)";
          } else {
            settings.position = "openedPreview";
            var height = _this7.calcPreviewHeight(modal) + "px"; // translation depuis le bas de la fenêtre
          ...
          return _await(_this7.openTransition(modal, settings), function () {
            _this7.memory.btnClose.classList.add("is-ready-to-animate");

            _this7.memory.btnToggle.classList.add("is-ready-to-animate", "is-toggle-up");

            _this7.dispatch({
              type: "updateModalPosition",
              position: _this7.memory.settings.position
            });
          });
        } catch (e) {
          return Promise.reject(e);
      ...
      key: "openTransition",
      value: function openTransition(el, settings) {
        return new Promise(function (resolve) {
      ...
      key: "closeTransition",
      value: function closeTransition(el, settings) {
        return new Promise(function (resolve) {
          el.addEventListener("transitionend", function _f() {
      ...
      key: "bindListeners",
      value: function bindListeners(modal) {
        var index,
      ...
      key: "unbindListeners",
      value: function unbindListeners(modal) {
        ...
        for (var index = 0; index < this.memory.btns.length; index++) {
          var btn = this.memory.btns[index];
      ...
      value: function calcPreviewHeight(modal) {
        var body = modal.querySelector("." + this.settings.bodyClassName);
        var MapHeight = modal.parentElement.offsetHeight;
        var previewHeight = MapHeight - (body.offsetTop + 36);
