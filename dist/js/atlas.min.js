var Atlas = (function (exports) {
  ...
  */
  ...
  var gsap$2,
    ...
    return _lookup$1[_getID(typeof target === "string" ? _toArray$1(target)[0] : target)];
  },
    ...
    if (gsap$2) {
      ...
      _getCache$1 = gsap$2.core.getCache;
  ...
  var VelocityTracker = /*#__PURE__*/function () {
    function VelocityTracker(target, property) {
      ...
      _lookup$1[_getID(this.target)] = this;
      this._props = {};
    ...
    _proto.kill = function kill(shallow) {
      ...
      if (!shallow) {
        delete _lookup$1[_getID(this.target)];
      }
    };
  ...
  _isArray$2 = Array.isArray,
      _emptyFunc$2 = function _emptyFunc(p) {
    return p;
  },
      _bigNum$3 = 1e10,
      _tinyNum$1 = 1 / _bigNum$3,
      _checkPoint = 0.05,
    ...
    for (p in obj) {
      copy[p] = _isObject$2(v = obj[p]) ? _deepClone(v) : v;
    }
    ...
    if (_isObject$2(n)) {
      ...
      if ((radius || _bigNum$3) < _bigNum$3 && radius < Math.sqrt(absDif)) {
        return n;
      }
    } else {
      while (i--) {
    ...
    var endVar = curProp.end,
        adjustedEnd,
        p;
    max = isNaN(max) ? _bigNum$3 : max;
    min = isNaN(min) ? -_bigNum$3 : min;
    ...
    if (_isObject$2(end)) {
      ...
      adjustedEnd = end.calculated ? end : (_isFunction$2(endVar) ? endVar(end) : _getClosest(end, endVar, max, min, radius)) || end;
      ...
      adjustedEnd = _isFunction$2(endVar) ? endVar(end) : _isArray$2(endVar) ? _getClosest(end, endVar, max, min, radius) : parseFloat(endVar);
    }
    ...
    if (gsap$3) {
      _parseEase$1 = gsap$3.parseEase;
      _toArray$2 = gsap$3.utils.toArray;
      _getUnit$1 = gsap$3.utils.getUnit;
      _getCache$2 = gsap$3.core.getCache;
      _clamp$1 = gsap$3.utils.clamp;
      _power3 = _parseEase$1("power3");
      _checkPointRatio = _power3(0.05);
      PropTween$1 = gsap$3.core.PropTween;
      gsap$3.config({
        resistance: 100,
        unitFactors: {
          time: 1000,
      ...
      _config$1 = gsap$3.config();
      gsap$3.registerPlugin(VelocityTracker);
      _coreInitted$3 = 1;
    }
  };
  ...
  var InertiaPlugin$1 = {
    ...
    init: function init(target, vars, tween, index, targets) {
      ...
      var cache = target._gsap,
          getVal = cache.get,
          dur = vars.duration,
          durIsObj = _isObject$2(dur),
          preventOvershoot = vars.preventOvershoot || durIsObj && dur.overshoot === 0,
          resistance = _getNumOrDefault(vars, "resistance", _config$1.resistance),
          duration = _isNumber$1(dur) ? dur : _calculateTweenDuration(target, vars, durIsObj && dur.max || 10, durIsObj && dur.min || 0.2, durIsObj && "overshoot" in dur ? +dur.overshoot : preventOvershoot ? 0 : 1, true),
          p,
          curProp,
          curVal,
          unit,
      ...
      for (p in vars) {
        if (!_reservedProps$1[p]) {
          curProp = vars[p];
          _isFunction$2(curProp) && (curProp = curProp(index, target, targets));

          if (_isNumber$1(curProp)) {
            velocity = curProp;
          } else if (_isObject$2(curProp) && !isNaN(curProp.velocity)) {
          ...
          this._pt = new PropTween$1(this._pt, target, p, curVal, 0, _emptyFunc$2, 0, cache.set(target, p, this));
          this._pt.u = unit || 0;
          this._pt.c1 = change1;
          this._pt.c2 = change2;
        }
  ...
  _getGSAP$2() && gsap$3.registerPlugin(InertiaPlugin$1);

  gsapWithCSS.registerPlugin(Draggable, InertiaPlugin$1);
  var Modal = /*#__PURE__*/function () {
    function Modal(id) {
      ...
      this.isOpen = false;
      this.isOpenFull = false; // Draggable

      var self = this; // gsap.config({ units: { top: "%" } });

      Draggable.zIndex = config.modal.zIndex; // TODO à garder pour masquer la modal ?
      // gsap.set(this._modal, { y: "100%" });

      var bounds = this._applyBounds();

      this.isOpen = true; //! Test uniquement

      var seuil = 100;
      console.log(bounds);
      var dragModal = Draggable.create(self._modal, {
        type: "y",
        ...
        bounds: bounds,
        onMove: function onMove(e) {
          console.log(this); // let direction = this.getDirection();
    ...
    _createClass(Modal, [{
      key: "_applyBounds",
      value: function _applyBounds(draggableEl) {
        var minVal = this._calcPreviewHeight();
      ...
      key: "_onDragToggleModal",
      value: function _onDragToggleModal(el, bool) {
        if (bool === true) {
          console.log("_onDragToggleModal isOpen (début/fermer): ", this.isOpen); // fermer
          ...
          console.log("_onDragToggleModal isOpen (fin/fermer): ", this.isOpen);
        } else {
          console.log("_onDragToggleModal isOpen (début/ouvrir): ", this.isOpen); // ouvrir
          ...
          this.isOpen = true;
          console.log("_onDragToggleModal isOpen (fin/ouvrir): ", this.isOpen);
          el.dataset.visibilityStatus = "all";
        }
      }
    }, {
      key: "_onClickToggleModal",
      value: function _onClickToggleModal(event) {
        ...
        if (this._modal.dataset.visibilityStatus === "preview") {
          ...
          this._modal.style.transform = "translateY(0)"; // this._modal.removeEventListener("click", this._clickListener);
          ...
          this.isOpen = false;
          console.log("_onClickToggleModal isOpen (fin/cas all): ", this.isOpen);
        }
      }
    }, {
      key: "open",
      value: function open(data) {
        ...
        console.log("open isOpen (debut): ", this.isOpen); // si déjà ouvert, fermer avant de charger le html
      ...
      value: function close() {
        ...
        return new Promise(function (resolve, reject) {
          console.log("close isOpen (début): ", _this2.isOpen);
          ...
          var transitionendListener = function transitionendListener(event) {
            self._modal.removeEventListener("transitionend", transitionendListener);
          ...
          self._modal.addEventListener("transitionend", transitionendListener);
      ...
      value: function _buildModal(data) {
        ...
        var html = this._createHTML(properties); // Ajouter le contenu html dans le conteneur


        var htmlContainer = this._modal.querySelector(".mp-Modal_Content");

        htmlContainer.innerHTML = html; // Avant d'afficher, vérifier que les images de la modale
        ...
        imagesLoaded(this._modal, function () {
          var previewHeight = _this3._calcPreviewHeight();

          console.log(previewHeight); // this._modal.style.top = previewHeight + "px";

          _this3._modal.style.transform = "translateY(".concat(previewHeight, "px)");
          _this3._modal.dataset.visibilityStatus = "preview"; // this._modal.addEventListener("click", this._clickListener);
        });
      }
    }, {
      key: "_calcPreviewHeight",
      value: function _calcPreviewHeight() {
        var parentHeight = this._modal.offsetParent.offsetHeight;

        var bodyContent = this._modal.querySelector(".mp-OrgProfile_Body");

        var padding = 24;
        var previewHeight = parentHeight - (bodyContent.offsetTop + padding);
  ...
  var Map = /*#__PURE__*/function () {
    function Map(mapObj) {
      ...
      this._json_url;
      this.body = document.body; // Double click sur terminaux tactiles. Solution provisoire.
