(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Atlas = factory());
  ...
  */
  ...
  var Tabs = /*#__PURE__*/function () {
    function Tabs(id) {
      ...
      this._tabsPanels = this._container.querySelectorAll('[role="tabpanel"]'); // For easy reference
      ...
      this._tabs.forEach(function (tab, index) {
        tab.addEventListener("click", _this._clickListener);
        tab.addEventListener("keydown", _this._keydownListener);
        tab.addEventListener("keyup", _this._keyupListener);
        tab.addEventListener("focus", _this._focusListener);
        tab.index = index;
      });
    }
    ...
    _createClass(Tabs, [{
      key: "_clickEvent",
      value: function _clickEvent(event) {
      ...
      key: "_keydownEvent",
      value: function _keydownEvent(event) {
      ...
      key: "_keyupEvent",
      value: function _keyupEvent(event) {
        var key = event.keyCode;
      ...
      key: "_focusEvent",
      value: function _focusEvent(event) {
        var _this2 = this;

        var target = event.target;
        var delay = 300;
        setTimeout(function () {
          _this2._checkTabFocus(target);
        }, delay);
      ...
      key: "_determineOrientation",
      value: function _determineOrientation(event) {
        var key = event.keyCode;
      ...
      key: "_switchTabOnArrowPress",
      value: function _switchTabOnArrowPress(event) {
      ...
      key: "_activateTab",
      value: function _activateTab(tab, setFocus) {
        setFocus = setFocus || true; // Deactivate all other tabs

        this._deactivateTabs(); // Remove tabindex attribute
      ...
      key: "_deactivateTabs",
      value: function _deactivateTabs() {
        var _this3 = this;

        this._tabs.forEach(function (tab) {
          tab.setAttribute("tabindex", "-1");
          tab.setAttribute("aria-selected", "false");
          tab.removeEventListener("focus", _this3._focusListener);
        });

        this._tabsPanels.forEach(function (panel) {
          panel.setAttribute("hidden", "hidden");
        });
      }
    }, {
      key: "_focusFirstTab",
      value: function _focusFirstTab() {
        this._tabs[0].focus();
      }
    }, {
      key: "_focusLastTab",
      value: function _focusLastTab() {
        this._tabs[this._tabs.length - 1].focus();
      } // Only activate tab on focus if it still has focus after the delay

    }, {
      key: "_checkTabFocus",
      value: function _checkTabFocus(target) {
        console.log(this);
      ...
      key: "_onclickActivityKeyword",
      value: function _onclickActivityKeyword(event) {}
    }]);

    return Tabs;
  }();

  var emitEvents = function emitEvents(element, eventName, target) {
  ...
  */
  ...
  var Keywords = /*#__PURE__*/function () {
    function Keywords(containerElement) {
      var _this = this;
      ...
      this._deleteBtnListener = this._onclickDeleteBtn.bind(this); // Mots-clés activités

      this._activityKeywords = this._container.querySelectorAll("." + config.keywords.activities.liClassName);

      this._activityKeywords.forEach(function (keyword) {
        keyword.addEventListener("click", _this._onclickActivityKeyword);
      });
    ...
    _createClass(Keywords, [{
      ...
      value: function addKeyword(keyword) {
        ...
        var value = keyword.value; // Vérifier si le mot-clé n'a pas déjà été sélectionné

        if (this._keywordsValues.indexOf(value) == -1) {
          this._keywordsValues.push(value); // Afficher le mot-clé saisi


          var markup = this._addMarkup(label, value); // Mettre à jour la carte


          emitEvents(this._container, "update", this._keywordsValues);
        }
      }
    }, {
      key: "removeKeyword",
      value: function removeKeyword(keywordEl) {
        var keyword = keywordEl.dataset.value;

        var index = this._keywordsValues.indexOf(keyword);

        if (index !== -1) {
          this._keywordsValues.splice(index, 1); // Si la liste est vide, supprimer <ul>
      ...
      key: "_onclickDeleteBtn",
      value: function _onclickDeleteBtn(event) {
        var btn = event.target;
        btn.removeEventListener("click", this._deleteBtnListener);
        var keywordEl = btn.parentElement;
        this.removeKeyword(keywordEl);
      }
    }, {
      key: "_addMarkup",
      value: function _addMarkup(label, value) {
        // <li>
        var li = document.createElement("li");
        li.className = config.keywords.liClassName;
        li.dataset.value = value; // <span>

        var span = document.createElement("span");
        var classStr = config.keywords.labelClassName; // La clé permet de déterminer l'icone de la catégorie à afficher
        ...
        span.className = classStr + " " + classStr + category; // Tronquer un intitulé excédent un nombre de caractères défini
        ...
        */
        ...
        li.appendChild(btn); // Identifier le conteneur des mots-clés

        var container = this._container.querySelector(config.keywords.containerId); // Une liste est déjà présente ?
  ...
  var SearchBox = /*#__PURE__*/function () {
    function SearchBox(id) {
      ...
      this._advancedSearchIsOpen = false; // Tabs : initialisation

      this.tabs = new Tabs("tabs"); // Keywords : initialisation des critères de recherche
    ...
    _createClass(SearchBox, [{
      ...
      value: function _initLayout(container) {
        ...
        this._cancelBtns.forEach(function (btn, index) {
          var uid = btn.dataset.relId;
          var input = container.querySelector("[data-id='" + uid + "']");
          btn.addEventListener("click", _this._onclickCancelBtn.bind(input));
      ...
      key: "_setHiddenState",
      value: function _setHiddenState(element, hidden) {
        element.setAttribute("aria-hidden", hidden);
      }
    }, {
      key: "_toggleSearchBoxPanels",
      value: function _toggleSearchBoxPanels() {
        var _this2 = this;
        ...
        var openPanel = this._panels[panelToShow]; // Animer l'un après l'autre

        var close = new Promise(function (resolve, reject) {
          var onhide = function onhide(event) {
            _this2._toggleAriaHiddenState(closePanel, true);

            _this2._toggleAriaBusyState(closePanel, false);

            closePanel.removeEventListener("transitionend", onhide);
          };

          closePanel.addEventListener("transitionend", onhide);
          closePanel.style.height = closePanel.getBoundingClientRect().height + "px";

          _this2._toggleAriaBusyState(closePanel, true);
        ...
        close.then(function () {
          var onshow = function onshow(event) {
            ...
            _this2._toggleAriaBusyState(openPanel, false);

            openPanel.removeEventListener("transitionend", onshow);
          };

          _this2._toggleAriaBusyState(openPanel, true);

          _this2._toggleAriaHiddenState(openPanel, false);

          openPanel.addEventListener("transitionend", onshow);
          openPanel.offsetHeight; // reflow

          var scrollHeight = openPanel.scrollHeight;
          openPanel.style.height = scrollHeight + "px";
        }, openPanel);
      }
    }, {
      key: "_toggleAriaBusyState",
      value: function _toggleAriaBusyState(element, bool) {
        element.setAttribute("aria-busy", bool);
      }
    }, {
      key: "_toggleAriaHiddenState",
      value: function _toggleAriaHiddenState(element, bool) {
  ...
  var AtlasBase = /*#__PURE__*/function () {
    ...
    _createClass(AtlasBase, [{
      ...
      value: function _initEvents() {
        ...
        this._container.addEventListener("update", function (event) {
          console.log(_this, event, event.detail);
        });
      }
    }]);
