var Ata = (function (exports) {
	...
	var setupSearchBox = function setupSearchBox() {
	  // Conteneur principal
	  var searchBox = document.querySelector(config.mapSearchBox.containerClass); // Labels

	  var labels = searchBox.querySelectorAll("label");
	  ...
	  var inputs = searchBox.querySelectorAll("input[type=text]");
	  var inputsIdArray = [];
	...
	*/
	...
	var handleClickOnMarker = function handleClickOnMarker(el) {
	  console.log(el); // prévoir de retirer et ajouter le gestionnaire
	};
	...
	*/
	...
	var mapInit = function mapInit(mapObj) {
	  _map.instance = mapObj; // _panes = new Panes("panes");

	  var modal = new Modal("Modal"); // Ajouter les boutons de zoom

	  setupControlZoom(mapObj); // Charger les points

	  ata_loadData(mapObj); // Initialiser la recherche
	...
	function ata_loadData(mapObj) {
	  ...
	  if (typeof map.options.json_points !== "undefined" && map.options.json_points.url.length) {
	    var args = {};
	...
	function ata_parseGeoJson(data) {
	  ...
	  if (!map.options.cluster) {
	    ...
	    var options = map.options.clusterOptions;
	    ...
	    for (var i = 0; i < data.features.length; i++) {
	      ...
	      if (feature.geometry.type == "Point" && feature.geometry.coordinates[0]) {
	        var marker = L.marker([feature.geometry.coordinates[1], feature.geometry.coordinates[0]]); // Déclarer l'icone du point

	        map.setGeoJsonFeatureIcon(feature, marker); // Pas de popup
	        // map.setGeoJsonFeaturePopup(feature, marker);
	        // Gérer le click sur le marker
	        // marker.addEventListener("click", handleClickOnMarker, false);

	        marker.on("click", handleClickMarker); // On garde en mémoire toute la feature d'origine dans le marker, comme sans clusters

	        marker.feature = feature; // Pour compat, on continue de garder l'id à part
	...
	function handleClickMarker(marker) {// _panes.open()
	...
	function ata_parseGeoJsonFeatures(data) {
	  ...
	  if (data.features && data.features.length > 0) {
	    var geojson = L.geoJson("", {
	      style: map.options.pathStyles ? map.options.pathStyles : function (feature) {
	...
	function ata_parseGeoJson_search(keywords) {
	  ...
	  if (Array.isArray(keywords) && keywords.length > 0) {
	    ...
	    var query = $.param(queryMap);
	    var url = "http.api/collectionjson/associations?" + query; // Effacer les points affichés sur la carte

	    map.removeAllMarkers(); // Collecter les associations qui répondent aux critères de recherche
	    ...
	    $.getJSON(url).done(function (json) {
	      ...
	      var collection = json.collection.items;

	      for (var _i = 0; _i < collection.length; _i++) {
	        associations.id.push(collection[_i].data[0].value);
	      }

	      var query = $.param(associations);
	      var url = "?page=gis_json&objets=associations_recherche&" + query;
	      $.getJSON(url).done(function (json) {
	    ...
	    }).fail(function () {// TODO Gérer l'absence de réponse
	    });
	  } else {
	    // Aucun critère de recherche, afficher la vue par défaut de la carte
	    // et recharger la totalité de la carte
	    map.removeAllMarkers();
	    map.setView([map.options.center[0], map.options.center[1]], map.options.zoom, {
	      animate: true
	    });
	    ata_loadData(map);
	  }
	...
	var keywordsValue = []; // Ajouter le mot-clé à la liste des critères de recherche
	// Fonction appelée par autocomplete_callback

	function addKeyword(keyword) {
	  if (keyword.label && keyword.value) {
	    var label = keyword.label;
	    var value = keyword.value; // Le mot-clé existe déjà dans le tableau principal

	    var alreadyExists = keywordsValue.indexOf(value);

	    if (alreadyExists == -1) {
	      keywordsValue.push(value); // index du mot dans le tableau principal
	      // let index = keywordsLabel.indexOf(label);

	      var item = keywordToItem(label, value);
	      var list = itemToList(item); // recharger la carte en fonction de la recherche
	...
	function deleteKeyword() {
	  ...
	  var index = keywordsValue.indexOf(value);
	...
	function keywordToItem(label, value) {
	  ...
	  listItem.className = config.keywords.itemClassName; // span element

	  var span = document.createElement("span"); // identifier la clé pour afficher l'icone de la catégorie

	  var key = value.split(":");
	  var category = keywordCategory(key[0]);
	  var className = config.keywords.labelClassName;
	  span.className = className + " " + className + category;
	  span.appendChild(document.createTextNode(label)); // button element

	  var btn = document.createElement("button");
	  btn.className = config.keywords.btnClassName;
	  btn.dataset.value = value;
	  btn.addEventListener("click", deleteKeyword, false); // append
	...
	function itemToList(item) {
	  ...
	  var listContainer = document.querySelector(config.keywords.containerId); // une liste de mots-clés est déjà présente ?

	  var list = listContainer.firstChild;
